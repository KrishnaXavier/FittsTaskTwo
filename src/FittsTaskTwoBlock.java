...
class FittsTaskTwoBlock
{
	...
	private double[] ide;
	private double[] tp;
	private int[] sequenceOrder; // this array controls the order of presenting
									// sequences

	FittsTaskTwoBlock(int numberOfSequencesArg, int numberOfTrialsArg, int[] aArg, int[] wArg, boolean randomize)
	{
		...
		// load the sequenceOrder array -- determines the order of presenting
		// sequences
		sequenceOrder = new int[numberOfSequences];
		Random r = new Random();
		for (int i = 0; i < sequenceOrder.length; ++i)
			sequenceOrder[i] = i; // sequential order (this is the order in the
									// configuration file)
		if (randomize) // if randomize = "yes" in the configuration file
			for (int i = 0; i < sequenceOrder.length; ++i)
			{
				...
				sequenceOrder[i] = tmp; // random order (more or less)
			}
	...
	{
		return FittsTaskTwoSequence.getSequenceHeader(); // presentation
	}
	...
	{
		// legacy code (didn't work anyway)
		// int order = 0;
		// while (sequenceOrder[order] != i)
		// ++order;
	 ...
	 *            a double array
	 * @param length
	 *            the length of the array (only the first 'length' values in the array are processed
	 ...
	 *            a double array
	 * @param length
	 *            the length of the array (only the first 'length' values in the array are
	 *            processed).
	 * @return a double equal to the standard deviation of the values in the array.
	 */
	...
	public String getBlockSummary()
	{
		...
		s += String.format("  MT = %.1f + %.1f IDe (r = %.4f)", this.getIntercept(), this.getSlope(), this
				.getCorrelation());
		return s;
	}
}
